    ...
    while stdin.read_line(&mut input).is_ok() {
        ...
        if let Some(command) = parse_input(input_string) {
            match command {
                ...
                ShellCommand::Echo(argument, _rout, _rerr, _rout_append, _rerr_append) => {
                    if _rout != "" {
                        ...
                        let mut f = if _rout_append {
                            fs::OpenOptions::new().append(true).create(true).open(_rout).expect("Unable to open file")
                        } else {
                            fs::OpenOptions::new().write(true).create(true).open(_rout).expect("Unable to open file")
                        };
                ...
                ShellCommand::Program(command, arguments, _rout, _rerr, _rout_append, _rerr_append) => {
                    ...
                    match command_type {
                        ...
                        CommandType::Program(_path) => {
                            ...
                            if _rout != "" {
                                ...
                                f.write_all(&output.stdout).expect("Unable to write to file");
                            } else {
                            ...
                            if _rerr != "" {
                                ...
                                let mut f = if _rerr_append {
                                    fs::OpenOptions::new().append(true).create(true).open(_rerr).expect("Unable to open file")
                                } else {
                                    fs::OpenOptions::new().write(true).create(true).open(_rerr).expect("Unable to open file")
                                };

                                f.write_all(&output.stderr).expect("Unable to write to file");
                            } else {
...
fn parse_input(input: &str) -> Option<ShellCommand> {
    ...
    match command.as_str() {
        "exit" => Some(ShellCommand::Exit(arguments.get(0).unwrap_or(&"0".to_string()).parse::<i32>().unwrap_or(0))),
        "echo" => { 
            ...
            Some(ShellCommand::Echo(arguments, rout, rerr, rout_append, rerr_append))
         },
        "type" => Some(ShellCommand::Type(arguments.get(0).unwrap_or(&"".to_string()).to_string())),
        "pwd" => Some(ShellCommand::Pwd()),
        "cd" => Some(ShellCommand::Cd(arguments.get(0).unwrap_or(&"".to_string()).to_string())),
        ...
        _default => { 
            ...
            Some(ShellCommand::Program(command, arguments, rout, rerr, rout_append, rerr_append)) 
        },
    }
}
